...
@author Cici Zheng, Harper Cho, Sharif Saleki

Helper functions for RSA, PCA, and MDS

Copyright 2022 Sharif Saleki (sharif.saleki@gmail.com).  This work is licensed
...
def calc_cross_similarity(dff_vals_container):

    '''
    Pearson correlation matrix and the angles for mean response vectors of all stimuli across 3 sessions.
    Args:
    ...
    Returns:
        across_session_corrs(np arr)    : (n_sessions*n_divs, n_sessions*n_divs), correlation coefficients of every two mean response vectors
        across_session_angles(np arr)   : (n_sessions*n_divs, n_sessions*n_divs), angles in degrees of every two mean response vectors
    '''
    ...
    for session_idx in range(n_sessions):
        avg_dff_vals_container.append(np.mean(
            dff_vals_container[session_idx][:, :, :],
            axis = 0
        ))
